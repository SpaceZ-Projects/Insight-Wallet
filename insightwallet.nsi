...
;Interface Settings
  ...
  !define MUI_ICON "build\insightwallet\windows\app\icon.ico"
...
Section "Install"
  ...
  File /r "build\insightwallet\windows\app\src\*.*"
  File "build\insightwallet\windows\app\icon.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\InsightWallet.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\InsightWallet.exe" "" "$INSTDIR\InsightWallet.exe" 0
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\app\insightwallet\resources\insightwallet.ico"
...
Function LaunchApp
  Exec "$INSTDIR\InsightWallet.exe"
FunctionEnd